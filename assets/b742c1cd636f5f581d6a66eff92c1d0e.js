...
$.cookie(key,'',$.extend({},options,{expires:-1}));return!$.cookie(key);};}));$(window).on('load',function(){window.location.hash="";});var screenWidth=$(window).width();if(screenWidth<=640){$('body').addClass('mobile');}
else{$('body').removeClass('mobile');}
var isMobile=$('body').hasClass('mobile');function resizeFirstSection(){var winheight=$(window).height();var winWidth=$(window).width();var firstSection=$('.inner');var firstSectionOnly=$('#intro .inner');var cocktails=$('.cocktails .inner');if(isMobile==false){firstSection.css({'min-width':winWidth+'px','height':winheight+'px'});cocktails.css({'min-width':winWidth+'px','height':winheight+'px'});}
if(isMobile==true){firstSectionOnly.css({'min-width':winWidth+'px','height':winheight+'px'});}}
resizeFirstSection();$('.video-wrapper').fitVids();$('.menu a[href*=#]:not([href=#])').click(function(e){var newUrl=$(this).data('url');if(location.pathname.replace(/^\//,'')==this.pathname.replace(/^\//,'')&&location.hostname==this.hostname){var target=$(this.hash);target=target.length?target:$('[name='+this.hash.slice(1)+']');if(target.length){$('html,body').stop().animate({scrollTop:target.offset().top},300,'easeInOutCubic',function(){$('.menu').removeClass('open');history.pushState("hello","foo",'#'+newUrl);});return false;}}});$('.menu-toggle-button-wrapper, .menu-closer').click(function(){$('.menu').toggleClass('open');});$(document).ready(function(){$.waitForImages.hasImgProperties=['backgroundImage']
$('#intro').waitForImages(function(){$(this).addClass('loaded');},$.noop,true);$(".royalSlider").royalSlider({keyboardNavEnabled:true,navigateByClick:false,arrowsNav:false});});$(window).on('resize',function(){resizeFirstSection();var screenWidth=$(window).width();var isMobile=$('body').hasClass('mobile');if(screenWidth<=640){$('body').addClass('mobile');}
else{$('body').removeClass('mobile');}});function isChecked(){if(!this.form.checkbox.checked){alert('You must agree to the terms and conditions to continue.');return false;}}
$('#age-gate-form').validate({rules:{"Month_form":"required","Day_form":"required","Year_form":"required","checkbox":"required"},submitHandler:function(form){ageGateValidation();}});function ageGateValidation(){var a=$('#Country_form').val();var d=$('#Day_form').val();var m=$('#Month_form').val();var y=$('#Year_form').val();var t=new Date();var td=t.getDate();var tm=t.getMonth()+1;var ty=t.getFullYear()-a;if(y>ty){alert('Give it a couple years');return false;}
else{alert('You can drink');$.cookie('the_cookie','the_value',{expires:7});$('.age-gate-background').addClass('gate-hidden');}
return true;}
if($.cookie('the_cookie')!=null){$('.age-gate-background').addClass('gate-hidden');}
else{}